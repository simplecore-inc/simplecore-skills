...
| a screen holds up for the person whose work it carries — a label cut at an edge, a control nobody can see, a font with no glyph, the longest language overflowing | **the coordinator**, opening every capture of the run once at the close, as that persona | a test asserts what it was told to; the class of failure that has no witness except the screen is the reason for looking |
