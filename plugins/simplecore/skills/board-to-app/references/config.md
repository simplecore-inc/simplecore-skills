...
| `journeyRoute` | where the running application itself opens — the address every journey test drives, as against the frame route, which renders one screen in one state | ○ | a journey test is driven wherever the run happens to be, and one driven at the frame route presses controls that have nowhere to go without anything erroring |
