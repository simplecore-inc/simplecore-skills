...
export async function inspectBoard(boardDir, { framePrefix = '' } = {}) {
  ...
  // Every file the board writes, in turn. A board that declares no split has one, and this is
  // the sweep it always was; a split board is swept whole, because a promise like 「nothing
  // scrolls sideways」 is about the board and one of its files cannot answer for it.
  ...
  for (const board of files) {
    ...
    const report = await page.evaluate(measureFrames, framePrefix);
    errors.push(...report.errors.map((l) => where + l));
    notes.push(...report.notes.map((l) => where + l));
