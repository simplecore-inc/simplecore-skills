...
| baseline (typography) | 글자 기준선 | 베이스라인, 밑선 | the line the glyphs of a row of text sit on, which is what an alignment option named `baseline` lines up. The qualifier is what keeps it apart from a measurement baseline: 「기준선」 on its own is the placeholder noun `references/response-style.md` §3 refuses, and a comparison baseline is written as the measured object (도입 전 처리기간 측정값). Hit: 「베이스라인에 맞춥니다」. Miss: 「도입 전 처리기간 측정값과 비교한다」, and the code identifier `alignItems="baseline"`, which stays as written |
...
| hyphen | 붙임표 | 하이픈, /대시(?!보드)/ | the `-` character, by its orthographic name, for a list marker or a range in Korean text. The em dash `—` is a different character and is refused outright by the rule `em-dash`; 「대시보드」 is a different word and is not caught. Hit: 「붙임표(-)로 표시합니다」. Miss: 「대시보드에 표시합니다」, a CLI flag such as `--only`, and an English range such as `pages 3-5` |
