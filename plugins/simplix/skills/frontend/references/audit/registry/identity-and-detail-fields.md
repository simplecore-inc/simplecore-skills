...
A cross-detail reference opens the referenced record in a `DetailPeekDialog`; its trigger is always `PeekTriggerButton`, never a hand-rolled `<Button>` with a `stopPropagation` closure. Two forms via the `appearance` prop: `"inline"` (default — outline button with the label text, next to a labeled value) and `"icon"` (compact icon-only ghost button carrying the label as tooltip / accessible name, for a `CrudList.Column`/`CrudDetail.Section` header `trailing` slot). Both use the `EyeIcon` and stop row-click propagation. A module-local icon-only peek button is a duplicate — use `appearance="icon"`.
