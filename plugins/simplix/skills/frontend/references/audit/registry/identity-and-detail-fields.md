...
A cross-detail reference opens the referenced record in a `DetailPeekDialog`; its trigger is always `PeekTriggerButton`, never a hand-rolled `<Button>` with a `stopPropagation` closure. Two forms via the `appearance` prop, and **the place decides which**, never the author: `"inline"` (default — outline button carrying the label and the icon; for a card, a footer action row, and a value that already has a label of its own beside it) and `"icon"` (icon-only ghost button with the label as tooltip and accessible name; for a `CrudList.Column`, a `DetailListRow`, and a `CrudDetail.Section` header `trailing` slot — anywhere the same control repeats down the page and a word on each would drown the values it sits beside). A separate `tight` prop decides whether the control hugs the value in front of it; it defaults to the shape (`"icon"` hugs, `"inline"` does not) and is overridden only for an icon in a trailing slot, where nothing precedes it. Every trigger also passes `target` — what it opens, as a person reads it — so the accessible name is 「남부현장 보기」 rather than the fourth 「보기」 in a column. Both draw the external-link icon — the dialog is a window onto another record, and an eye says 「read-only」, which is a different promise — and both stop row-click propagation. A module-local icon-only peek button is a duplicate — use `appearance="icon"`.
