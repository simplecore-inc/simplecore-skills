...
These run over EVERY screen in scope, exhaustively — not sampled. The first four have detection recipes in `references/screen-checklist.md`; the fifth runs over the cluster in `references/cross-screen-consistency.md`; the sixth is the scripted checks in `references/browser-driving.md`.
...
6. **Rendered, not merely served.** While the screen is open, run `audit-rendered.mjs`'s checks on it: a list total stating N rows over a column that draws none, two pieces of text painted into one rectangle, an open pane that paints nothing, and a list scrolling inside itself while the page around it holds still. **The list is what `--list` prints, never this sentence** — a subset named in prose is how a reader runs three of seven and reports the rest as clean. All of them survive every other census — the strings are right, the requests answered, the components are imported — and all of them make the screen unusable. A reader scanning a screenshot for a row that is not there is looking for an absence, which is the one thing eyes are worst at; these settle it in one evaluate call. **The pane check needs the emptied state to have anything to report**, so open each tabbed screen with its reads emptied as well as populated → `references/browser-driving.md` § The checks that need the browser.
